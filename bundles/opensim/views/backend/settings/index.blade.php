                ...
                @if(isset($setting->type) and !empty($setting->type))
                    ...
                    @else
                        <div class="control-group {{ $errors->has($setting->slug) ? 'error' : '' }}">
                            <label for="{{ $setting->slug }}" class="control-label">{{ Lang::line('opensim::lang.'.trim($setting->title))->get(ADM_LANG) }}</label>
                            <div class="controls">
                                {{ call_user_func("\Form::mwi_field", $setting) }}
                                @if($setting->type == 'text')
                                <span class="required-icon"></span>
                                @endif
                                <span class="help-inline">{{ $errors->has($setting->slug) ? $errors->first($setting->slug, '<small style="color:#dd3c10;">:message</small>') : Lang::line('opensim::lang.'.trim($setting->description))->get(ADM_LANG) }}</span>
