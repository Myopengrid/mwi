<?php

return array(

	/*
	|--------------------------------------------------------------------------
	| Default Swift Mailer Transport
	|--------------------------------------------------------------------------
	|
	| The name of your default Swift Mailer Transport. This transport will used
	| as the default for all mailing operations unless a different name is
	| given when performing said operation. This transport name should be
	| listed in the array of transports below.
	| options sendmail, smtp, mail
	|
	*/

	'default' => Config::get('settings::core.mail_protocol'),

	/*
	|--------------------------------------------------------------------------
	| Swift Mailer Transports
	|--------------------------------------------------------------------------
	|
	| Below is the configuration for each of the Swift Mailer transports. For
	| more information refer to:
	|
	|	http://swiftmailer.org/docs/sending.html
	| 
	|
	| If you want to use Gmail as your email transport, set
	|	'host'       =>	'smtp.gmail.com', 
	|	'username'   =>	'username@gmail.com',
	|	'password'   =>	'password',
	|	'port'       =>	465,
	|	'encryption' =>	'ssl', 'tls'
	*/

	'transports' => array(

		'smtp' => array(
			'host'       => Config::get('settings::core.mail_smtp_host'),
			'port'       => Config::get('settings::core.mail_smtp_port'),
			'username'   => Config::get('settings::core.mail_smtp_user'),
			'password'   => Config::get('settings::core.mail_smtp_pass'),
			'encryption' => null,
		),

		'sendmail' => array(
			'command' => Config::get('settings::core.mail_sendmail_path').' -bs',
		),

		'mail',

	),
);